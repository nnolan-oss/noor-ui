  ...
  (
    ...
    const handleValueChange = useCallback(
      (newValue: string | string[]) => {
        console.log("Accordion value change:", {
          disabled,
          newValue,
          currentValue: value,
        });
    ...
    const accordionSize = objectsToString(
      (sizes as any)[findMatch(valid.sizes, size, "md")]
    );

    const classes = twMerge(
      classnames(accordionBase, accordionSize, {
...
>(({ children, className, ...rest }, ref) => {
  const { value, onValueChange, type, collapsible, disabled } =
    useAccordionContext();
  ...
  const handleClick = () => {
    console.log("AccordionTrigger clicked:", {
      isDisabled,
      itemValue,
      isOpen,
      type,
  ...
  return (
    <AnimatePresence initial={false}>
      {isOpen && (
        ...
        >
          <div
            className={twMerge("p-4 pb-4 text-gray-600", className)}
            {...rest}
          >
            {children}
          </div>
